...
const TripTransactions: React.FC = () => {
    ...
    const getEventConfig = (eventType: string) => {
        ...
        switch (eventType) {
            case "trip_requested":
                return { title: "Ride Requested", color: "indigo", bg: "bg-indigo-50 dark:bg-indigo-500/10", ring: "ring-indigo-50/50", icon: <ClockCircleOutlined className="text-indigo-600" /> };
            case "trip_assigned":
                return { title: "Driver Assigned", color: "blue", bg: "bg-blue-50 dark:bg-blue-500/10", ring: "ring-blue-50/50", icon: <UserAddOutlined className="text-blue-600" /> };
            case "trip_started":
                return { title: "Journey Started", color: "emerald", bg: "bg-emerald-50 dark:bg-emerald-500/10", ring: "ring-emerald-50/50", icon: <PlayCircleOutlined className="text-emerald-600" /> };
            case "trip_completed":
            ...
            case "trip_cancelled":
                return { title: "Trip Cancelled", color: "rose", bg: "bg-rose-50 dark:bg-rose-500/10", ring: "ring-rose-50/50", icon: <StopOutlined className="text-rose-600" /> };
            default:
                return { title: eventType.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()), color: "slate", bg: "bg-slate-50 dark:bg-slate-800", ring: "ring-slate-50/50", icon: <EnvironmentOutlined className="text-slate-600" /> };
        }
    };
            ...
            <div className="p-8 pb-32 max-w-[1600px] mx-auto space-y-8">
                ...
                {loading && (
                    <div className="flex flex-col items-center justify-center py-24 gap-4">
                        <Spin size="large" />
                        <span className="text-xs font-black uppercase tracking-[0.2em] text-gray-400 dark:text-slate-500 dark:text-slate-400">Synchronizing Ledger...</span>
                    </div>
                )}
                ...
                {hasSearched && !loading && !tripData?.transactions?.length && !error && (
                    <div className="text-center py-24 opacity-40 grayscale">
                        ...
                        <p className="mt-4 text-xs font-black uppercase tracking-widest text-gray-400 dark:text-slate-500 dark:text-slate-400">Empty Record Set</p>
                    </div>
                )}
                        ...
                        <div className="bg-white rounded-[2.5rem] border border-gray-200 shadow-sm overflow-hidden">
                            <div className="px-8 py-6 border-b border-gray-200 dark:border-slate-700 bg-gray-50/30 dark:bg-slate-800/50 flex flex-wrap items-center justify-between gap-4">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-xl font-black text-gray-800 dark:text-slate-200 tracking-tight leading-none mb-1">Trip Lifecycle</h3>
                                        <div className="flex items-center gap-2">
                                            <span className="text-[10px] bg-slate-100 text-slate-500 dark:text-slate-400 px-2.5 py-1 rounded font-mono font-bold tracking-tighter border border-slate-200">ID: {searchId}</span>
                                            <span className="text-[10px] bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 px-2.5 py-1 rounded font-bold uppercase tracking-widest border border-indigo-100 dark:border-indigo-500/30">{tripData.total} Events Recorded</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-6 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-[1.5rem] p-4">
                                    <div className="flex items-center gap-3 pr-6 border-r border-gray-100 dark:border-slate-700">
                                        <Avatar className="bg-gradient-to-tr from-amber-400 to-orange-300 border-2 border-white dark:border-slate-800 w-10 h-10" icon={<UserOutlined />} />
                                        <div className="flex flex-col">
                                            <span className="text-[9px] text-gray-400 dark:text-slate-500 dark:text-slate-400 font-black uppercase tracking-wider">Rider</span>
                                            <span className="text-xs font-extrabold text-gray-700 dark:text-slate-200 leading-none">{tripData.user?.name || "N/A"}</span>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <Avatar className={`w-10 h-10 border-2 border-white dark:border-slate-800 ${tripData.driver ? "bg-gradient-to-tr from-emerald-400 to-teal-300" : "bg-gray-100 dark:bg-slate-700"}`} icon={<CarOutlined />} />
                                        <div className="flex flex-col">
                                            <span className="text-[9px] text-gray-400 dark:text-slate-500 dark:text-slate-400 font-black uppercase tracking-wider">Driver Partner</span>
                                            <span className="text-xs font-extrabold text-gray-700 dark:text-slate-200 leading-none">{tripData.driver?.name || "Assign Pending..."}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        return (
                                            <div key={txn.id ?? idx} className="relative flex flex-col sm:flex-row gap-8 sm:items-start group">
                                                ...
                                                <div className="sm:w-32 pt-2 text-right hidden sm:block">
                                                    <span className="text-[10px] font-black text-gray-300 dark:text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] block leading-none mb-1">Recorded At</span>
                                                    <span className="text-xs font-bold text-gray-500 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 tracking-tight">{dayjs(txn.event_at).format("hh:mm A")}</span>
                                                    <span className="text-[9px] font-medium text-gray-400 dark:text-slate-500 dark:text-slate-400 block mt-1">{dayjs(txn.event_at).format("DD MMM · YYYY")}</span>
                                                </div>

                                                {/* Center: Bubble */}
                                                <div className="relative z-10 hidden sm:block">
                                                    <div className={`w-14 h-14 rounded-[1.25rem] border-4 border-white dark:border-slate-800 flex items-center justify-center ${config.bg} transition-transform group-hover:scale-110`}>
                                                        {config.icon}
                                                    </div>
                                                </div>
                                                    ...
                                                    <div className="flex flex-col lg:flex-row gap-6">
                                                        ...
                                                        <div className="shrink-0 min-w-[200px]">
                                                            <div className="flex items-center gap-3 mb-2">
                                                                <h4 className="text-base font-black text-gray-800 dark:text-slate-200 tracking-tight">{config.title}</h4>
                                                                <span className={`px-2 py-0.5 rounded-lg text-[10px] font-extrabold tracking-widest uppercase border ${txn.status === 'success' ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-500/30' : 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-100 dark:border-rose-500/30'}`}>
                                                                    {txn.status}
                                                                </span>
                                                            </div>
                                                            ...
                                                            {txn.notes && (
                                                                <div className="flex-1 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-4 shadow-sm flex items-center min-w-[240px]">
                                                                    <div className="flex flex-col gap-1 w-full">
                                                                        <span className="text-[9px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest leading-none">Activity Note</span>
                                                                        <Text className="text-xs font-bold text-gray-700 dark:text-slate-200 leading-tight italic">“{txn.notes}”</Text>
                                                                    </div>
                                                                </div>
                                                            )}
                                                            ...
                                                            {txn.event_type === "trip_requested" && txn.entity_snapshot && (
                                                                <div className="flex-[2] grid grid-cols-2 gap-4 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm">
                                                                    <div className="flex items-center gap-3 pr-4 border-r border-gray-50 dark:border-slate-700">
                                                                        <div className="w-8 h-8 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-sm shrink-0"><EnvironmentOutlined /></div>
                                                                        <div className="flex flex-col min-w-0">
                                                                            <span className="text-[9px] font-black text-gray-300 dark:text-slate-500 dark:text-slate-400 uppercase tracking-wider leading-none mb-1">Pickup</span>
                                                                            <span className="text-xs font-bold text-gray-700 dark:text-slate-200 leading-none truncate max-w-[150px]">{txn.entity_snapshot.pickup_address || "Specified Location"}</span>
                                                                        </div>
                                                                    </div>
                                                                    <div className="flex items-center gap-3 pl-2">
                                                                        <div className="w-8 h-8 rounded-xl bg-rose-50 dark:bg-rose-500/10 flex items-center justify-center text-rose-600 dark:text-rose-400 shadow-sm shrink-0"><EnvironmentOutlined /></div>
                                                                        <div className="flex flex-col min-w-0">
                                                                            <span className="text-[9px] font-black text-gray-300 dark:text-slate-500 dark:text-slate-400 uppercase tracking-wider leading-none mb-1">Endpoint</span>
                                                                            <span className="text-xs font-bold text-gray-700 dark:text-slate-200 leading-none truncate max-w-[150px]">{txn.entity_snapshot.drop_address || "Specified Location"}</span>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            )}
                                                        ...
                                                        <div className="pt-4 border-t border-gray-100 dark:border-slate-700 mt-2">
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                                {Object.entries(txn.new_value ?? {}).map(([key, val]) => (
                                                                    <div key={key} className="flex flex-col bg-white/50 dark:bg-slate-800/50 p-3 rounded-xl border border-gray-100/50 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-800 transition-colors group/audit">
                                                                        <span className="text-[9px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 group-hover/audit:text-indigo-400 transition-colors uppercase-font tracking-wide-font">{FIELD_LABELS[key] || key.replace(/_/g, " ")}</span>
