  ...
  const columns = [
    {
      ...
      key: 'created_at',
      render: (date: string) => <span className="text-xs text-slate-500 font-medium">{dayjs(date).format('DD MMM YYYY, HH:mm')}</span>,
    },
    {
      title: 'Driver',
      key: 'driver',
      render: (_: any, record: any) => (
        <div className="flex flex-col">
          <span className="text-sm font-bold text-slate-800 dark:text-slate-200">{record.driver_name}</span>
    ...
    },
    {
      ...
      dataIndex: 'plan_name',
      key: 'plan_name',
      render: (plan: string) => <span className="font-semibold text-slate-700 dark:text-slate-300">{plan}</span>,
    },
    {
      title: 'Amount',
      dataIndex: 'amount',
    ...
    },
    {
      ...
      render: (status: string) => (
        <Tag color={status === 'SUCCESS' ? 'success' : status === 'PENDING' ? 'warning' : 'error'} className="font-bold">
          {status}
        </Tag>
      ),
    },
    {
      ...
      key: 'payment_method',
      render: (method: string) => <span className="text-xs font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">{method}</span>,
    },
    {
      title: 'Actions',
      key: 'actions',
