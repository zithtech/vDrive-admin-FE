...
import { Modal, Button, Tag, Divider, message, Space, Select, Input, Alert, Spin, Typography } from "antd";
import { 
  CheckCircleOutlined, 
  CloseCircleOutlined, 
  ZoomInOutlined, 
...
const DriverApprovalModal: React.FC<DriverApprovalModalProps> = ({ driver, open, onClose }) => {
  ...
  useEffect(() => {
    if (selectedDoc && selectedDoc.status === "pending" && !selectedDoc.extracted_data && !isOCRRunning && !selectedDoc.isVehicle) {
  ...
  const handleVerifySingleDoc = async (status: string) => {
    ...
    try {
      setLoading(true);
      const driverId = driver.driver_id || driver.id || "";
      await dispatch(updateDocumentStatus({
        driver_id: driverId,
        document_id: selectedDoc.id,
        status: status,
    ...
    >
      <div className="p-6 pb-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black m-0 text-slate-800 dark:text-slate-100">Review Application</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium m-0">Awaiting Admin Approval</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex h-[65vh] min-h-[500px] border-t border-slate-100 dark:border-slate-800 mt-4 mx-6">
        <div className="w-1/3 border-r border-slate-100 dark:border-slate-800 pr-6 pt-4 flex flex-col h-full overflow-y-auto pb-6">
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 flex items-center gap-4 border border-slate-200 dark:border-slate-700">
            <img 
              src={getMediaUrl(driver.profile_pic_url) || "https://ui-avatars.com/api/?name=" + driver.full_name} 
              alt={driver.full_name} 
              className="w-14 h-14 rounded-full object-cover border-2 border-white dark:border-slate-700 shadow-sm"
            />
            <div>
              <h3 className="font-bold text-slate-800 dark:text-slate-100 m-0 leading-tight">{driver.full_name}</h3>
              ...
              <div className="flex gap-2 mt-1.5">
                <Tag color="blue" className="m-0 text-[10px] uppercase font-bold border-0">New Driver</Tag>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 flex flex-col gap-2">
            <div className="flex justify-between items-center text-[11px]">
              <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Driver ID</span>
            ...
            <div className="flex justify-between items-center text-[11px]">
              <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Alt Phone</span>
              <span className="text-slate-800 dark:text-slate-100 font-bold">{driver.alternate_contact || "N/A"}</span>
            </div>
            <div className="flex justify-between items-center text-[11px]">
              <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Email</span>
              <span className="text-slate-800 dark:text-slate-100 font-bold truncate max-w-[140px]" title={driver.email}>{driver.email || "N/A"}</span>
            </div>
            <div className="flex justify-between items-center text-[11px]">
              <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">DOB</span>
              <span className="text-slate-800 dark:text-slate-100 font-bold">{driver.dob || (driver as any).date_of_birth || "N/A"}</span>
            </div>
            <div className="flex justify-between items-center text-[11px]">
              <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Gender</span>
              <span className="text-slate-800 dark:text-slate-100 font-bold capitalize">{driver.gender || "N/A"}</span>
            </div>
            <div className="flex justify-between items-center text-[11px] pt-1.5 border-t border-slate-50 dark:border-slate-700/50">
              <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Role</span>
              <span className="text-slate-800 dark:text-slate-100 font-bold capitalize">{driver.role || "Normal"}</span>
            </div>
            <div className="flex justify-between items-center text-[11px]">
              <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Status</span>
              <Tag className="m-0 border-0 font-bold text-[10px] uppercase" color={driver.status === 'active' ? 'green' : 'orange'}>
                {driver.status}
              </Tag>
            </div>
            <div className="flex flex-col gap-1 text-[11px] pt-1.5 border-t border-slate-50 dark:border-slate-700/50">
              <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Full Address</span>
            ...
            <div className="flex flex-col gap-1 text-[11px] pt-1.5 border-t border-slate-50 dark:border-slate-700/50">
              <div className="flex justify-between items-center">
                <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Joined</span>
                <span className="text-slate-800 dark:text-slate-100 font-bold">{driver.created_at ? dayjs(driver.created_at).format("DD MMM YYYY, hh:mm A") : "N/A"}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Last Update</span>
                <span className="text-slate-800 dark:text-slate-100 font-bold">{driver.updated_at ? dayjs(driver.updated_at).format("DD MMM YYYY, hh:mm A") : "N/A"}</span>
              </div>
            </div>
          </div>
          ...
          {driver.vehicle && (
            <div className="mt-3 bg-orange-50/50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/30 rounded-xl p-3 flex flex-col gap-2">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-black text-orange-600 dark:text-orange-500 uppercase tracking-widest">Vehicle Details</span>
              </div>
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Number</span>
                <span className="text-slate-800 dark:text-slate-100 font-mono font-bold uppercase">{driver.vehicle.vehicle_number || "N/A"}</span>
              </div>
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Model</span>
                <span className="text-slate-800 dark:text-slate-100 font-bold">{driver.vehicle.vehicle_model || "N/A"}</span>
              </div>
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-slate-500 dark:text-slate-400 font-medium uppercase tracking-widest">Type / Fuel</span>
                <span className="text-slate-800 dark:text-slate-100 font-bold capitalize">{driver.vehicle.vehicle_type || "N/A"} / {driver.vehicle.fuel_type || "N/A"}</span>
              </div>
            </div>
          )}

          <Divider className="my-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Documents ({combinedDocs.length})</Divider>
          ...
          <div className="flex flex-col gap-2 flex-grow">
            {combinedDocs.map(doc => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${doc.isVehicle ? 'bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400' : 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400'}`}>
                    {doc.isVehicle ? <CarOutlined /> : <FileTextOutlined />}
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-slate-700 dark:text-slate-200 m-0 leading-none capitalize">{doc.type.replace(/_/g, ' ')}</h4>
                    {!doc.type.toLowerCase().includes('selfie') && !doc.type.toLowerCase().includes('police') && (
                ...
                <div className="flex flex-col items-end gap-1">
                  {doc.status === 'verified' && <CheckCircleOutlined className="text-emerald-500 text-lg" />}
                  {doc.status === 'rejected' && <CloseCircleOutlined className="text-rose-500 text-lg" />}
                  {(doc.status === 'pending' || !doc.status) && <div className="w-2 h-2 rounded-full bg-amber-400" />}
                  <Button 
                    type="link" 
                    size="small" 
            ...
            <>
                ...
                {!selectedDoc.isVehicle && (
                  isRejectingDoc ? (
                    <Space>
                      ...
                      />
                      <Button size="small" onClick={() => { setIsRejectingDoc(false); setDocRejectReason(""); }}>Cancel</Button>
                      <Button size="small" danger type="primary" onClick={() => handleVerifySingleDoc('rejected')} loading={loading}>Confirm Reject</Button>
                    </Space>
                  ) : (
                    <Space>
                      <Button size="small" danger onClick={() => setIsRejectingDoc(true)} loading={loading}>Reject</Button>
                      <Button size="small" type="primary" className="bg-emerald-500" onClick={() => handleVerifySingleDoc('verified')} loading={loading}>Verify</Button>
                    </Space>
                  )
                )}
              </div>
              ...
              <div className="flex-1 bg-slate-100 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 relative overflow-hidden flex items-center justify-center">
                ...
                {selectedDoc.url ? (
                  ...
                  <div className="text-slate-400 dark:text-slate-500 font-medium">No Image Available</div>
                )}
              </div>
              ...
              {selectedDoc.extracted_data && (
                <div className="mt-4 p-4 bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-900/30 rounded-xl">
                  ...
                  <div className={selectedDoc.type.toLowerCase().includes('license') || selectedDoc.type.toLowerCase().includes('dl') ? "grid grid-cols-2 gap-4" : "grid grid-cols-1 gap-4"}>
                    <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-purple-100/50 dark:border-purple-800/50">
                      <span className="text-xs text-slate-500 dark:text-slate-400 block">Detected Number</span>
                      <span className="font-semibold text-slate-800 dark:text-slate-100">{selectedDoc.extracted_data.extracted_number || 'N/A'}</span>
                    </div>
                    {(selectedDoc.type.toLowerCase().includes('license') || selectedDoc.type.toLowerCase().includes('dl')) && (
                      <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-purple-100/50 dark:border-purple-800/50">
                        <span className="text-xs text-slate-500 dark:text-slate-400 block">Detected Expiry</span>
                        <span className="font-semibold text-slate-800 dark:text-slate-100">{selectedDoc.extracted_data.extracted_expiry || 'N/A'}</span>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div className="border-t border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50 p-4 px-6 rounded-b-lg">
        {isRejecting ? (
          <div className="flex-1 flex items-center gap-3 w-full">
            ...
            <Button danger type="primary" onClick={handleRejectDriver} loading={loading}>Confirm Reject</Button>
          </div>
        ) : (
          <>
            <Button 
      ...
      <Modal
        title={
          <div className="flex items-center gap-2 text-gray-800 dark:text-slate-100">
            ...
            <span className="capitalize">Document History: {historyModalDoc?.type?.replace(/_/g, " ")}</span>
          </div>
        }
          ...
          {loading && documentHistory.length === 0 ? (
            ...
            <div className="space-y-4">
              {documentHistory.map((item, index) => (
                <div key={item.id || index} className="p-4 rounded-xl bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                      item.status === 'verified' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400'
                    }`}>
                      {item.status === 'verified' ? <CheckCircleOutlined /> : <CloseCircleOutlined />}
                    </div>
                    {index !== documentHistory.length - 1 && <div className="w-[1px] h-full bg-gray-200 dark:bg-slate-700 my-1" />}
                  </div>
                  <div className="flex-grow">
                    <div className="flex justify-between items-center mb-1">
                      <Tag color={item.status === 'verified' ? 'success' : 'error'} className="text-[10px] m-0 border-none px-2 rounded-lg uppercase font-bold">
                        {item.status}
                      </Tag>
                    ...
                    {item.reason && (
                      <div className="bg-white dark:bg-slate-900 p-2 rounded-lg border border-gray-100 dark:border-slate-700 mt-2">
                        <Typography.Text className="text-xs text-gray-600 dark:text-slate-300 italic">"{item.reason}"</Typography.Text>
                      </div>
                    )}
                  </div>
                </div>
