...
const TripManagement: React.FC<TripManagementProps> = ({ stats, trips }) => {
  ...
  const StatItem = ({
    ...
    <div className="flex items-center p-3 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl hover:shadow-sm transition-all duration-300 group cursor-default h-[68px]">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <Text className="text-[10px] text-gray-400 dark:text-gray-500 font-medium mb-0 tracking-tight">
          {title}
        </Text>
        <p className="m-0 text-gray-900 dark:text-white font-bold text-[17px] leading-tight">
          {stats.loading ? "..." : value}
        </p>
      </div>
    </div>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl shadow-sm flex flex-col h-105 overflow-hidden">
      ...
      {/* Stats Section */}
      <div className="p-3.5 border-b border-gray-50 dark:border-slate-700 flex flex-col gap-3">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-1.5 space-y-1">
          {trips.length > 0 ? (
            trips.map((trip) => (
              <div
                key={trip.trip_id}
                className="p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 transition-all group flex flex-col gap-2"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-gray-50 dark:bg-slate-700 flex items-center justify-center shrink-0">
                ...
                <div className="flex items-center justify-between pl-10.5">
                  <div className="flex items-center gap-1.5 text-gray-400 dark:text-gray-500 overflow-hidden">
