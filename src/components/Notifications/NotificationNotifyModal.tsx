...
const NotificationNotifyModal: React.FC<NotificationNotifyModalProps> = ({
  ...
  const [selectedUserIds, setSelectedUserIds] = useState<string[]>(
    notificationData?.specific_user_id || [],
  );
  ...
  return (
    <Modal
      ...
      footer={[
        <Button
          key="cancel"
          onClick={onCancel}
          className="rounded-xl border-none text-slate-400 font-bold hover:text-slate-600 dark:hover:text-slate-200 h-12 px-6"
        >
    ...
    >
      <div className="py-2">
        {!isAllowed && (
          <div className="mb-4 bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-[1.5rem] text-xs flex items-start gap-2 shadow-sm">
            ...
            <div>
              ...
              <span>
                You do not have permission to dispatch notification campaigns. Selection options
                have been locked.
              </span>
            </div>
          </div>
        ...
        <div className="mb-6">
          <Text className="text-slate-400 dark:text-slate-500 font-black text-[10px] uppercase tracking-widest mb-3 block px-1">
            Target Audience
          </Text>
          <Select
          ...
          >
              ...
              <div className="flex items-center gap-2 py-1">
                ...
                <div>
                  <div className="text-xs font-bold text-slate-700 dark:text-slate-200">
                    All {targetType === "CUSTOMER" ? "Customers" : "Drivers"}
                  </div>
                  <div className="text-[9px] text-slate-400 dark:text-slate-500 uppercase font-bold tracking-tight">
            ...
            <Option value="SPECIFIC">
              <div className="flex items-center gap-2 py-1">
                ...
                <div>
                  <div className="text-xs font-bold text-slate-700 dark:text-slate-200">
                    Specific {targetType === "CUSTOMER" ? "Customer" : "Driver"}
                  </div>
                  <div className="text-[9px] text-slate-400 dark:text-slate-500 uppercase font-bold tracking-tight">
          ...
          <div className="mb-6 animate-in fade-in slide-in-from-top-2 duration-500">
            ...
            <Select
              ...
              filterOption={(input, option) => {
                ...
                return (
                  userName.toLowerCase().includes(input.toLowerCase()) || userPhone.includes(input)
                );
              }}
            >
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700">
          <Text className="text-[11px] text-slate-500 dark:text-slate-400 italic block">
            <span className="font-bold text-slate-700 dark:text-slate-300">
              Pro-tip for achieving this:
            </span>{" "}
            The backend utilizes a scheduled cron job (running every 5 minutes) to process these
            requests. It queries the database based on your selected segments (e.g.,{" "}
            <Text code className="dark:bg-slate-800 dark:text-slate-300">
              total_rides &gt; 50
            </Text>{" "}
            for Top Riders) and dispatches notifications in batches to ensure maximum deliverability
